...
def run_rag_chain(
    ...
    classification_type: ClassificationType = ClassificationType.NEWQUESTION
    ...
    if "classificationChainConfig" in llm_config:
        ...
        ##########################
        classificationHistoryList = get_message_history(user_id=user_id, chat_id=chat_id, history_limit=6)
        classificationHistory = format_chat_history(classificationHistoryList)
        ##########################
        ...
        classification_response, input_tokens, output_tokens = (
            run_classification_step(
                ...
                classificationChatHistory=classificationHistory
            )
            or {}
        )
        ...
        if (classification_type == ClassificationType.UNRELATED):
            ...
            return {
                ...
                "sources": ai_message.get("documents"),
                "traceData": app_trace.get_trace(),
                "handoffTriggered": classification_response.get("handoff_state"),
            }
    standalone_q = user_q
    if ("standaloneChainConfig" in llm_config) and (classification_type != ClassificationType.FOLLOWUP):
    ...
    if classificationHistory:
        qachatHistory = classificationHistory
    else:
        qahistoryList = get_message_history(user_id=user_id, chat_id=chat_id, history_limit=10)
        qachatHistory = format_chat_history(qahistoryList)
    ##########################
    ...
    # Is a valid new question
    firstMessage = get_first_message(user_id=user_id,chat_id=chat_id,history_limit=1)
    print(f"First message: {firstMessage['messages']}")

    titleConfig = llm_config["qaChainConfig"]
    if ("standaloneChainConfig" in llm_config):
        titleConfig = llm_config["standaloneChainConfig"]
    elif ("classificationChainConfig" in llm_config):
        titleConfig = llm_config["classificationChainConfig"]

    initialTitle = firstMessage["title"]
    print(f"Initial Title {initialTitle}")
    if "New Chat - " in initialTitle['title']:
        summarized_title = generate_chat_title(
            chain_config=titleConfig,
            user_q=user_q
        )
        update_default_title(user_id=user_id,chat_id=chat_id,new_title=summarized_title,history_limit=1)
        print(f"Generated Title: {summarized_title}")
...
def run_qa_step(
    ...
    contextFromFollowUp = False
    if classification_type == ClassificationType.FOLLOWUP:
        sourcesFromFollowup = get_sources_for_followup(user_id=userID,chat_id=chatID,history_limit=1)
        documents.extend(sourcesFromFollowup)
        context = format_documents(documents)
        contextFromFollowUp = True
        print(f"Documents in run qa step from follow up: {documents}")
    else:
        ...
        if documents and not contextFromFollowUp:
            if reranking_config:
    ...
    if qaChatHistory is not None:
        kwargs["chat_history"] = qaChatHistory
    ...
    return (llm_response, documents, input_tokens, output_tokens)
...
def generate_chat_title(chain_config: dict, user_q: str) -> str:
    ...
    llm_response, input_tokens, output_tokens = llm.call_text_llms(
        model_config=model_config,
        prompt_template=f"Generate a 2-4 word summary of this user's question: {user_q}",
        prompt_variables=[],
        **kwargs,
    )
...
def run_classification_step(
    ...
    handoff_config: Optional[dict] = None,
    classificationChatHistory=None
) -> dict[str, str] | None:
    """
    ...
    kwargs = chain_config.get("kwargs", {})
    kwargs["question"] = question
    if classificationChatHistory is not None:
        kwargs["chat_history"] = classificationChatHistory
    ...
    classification_type = response.get("classification_type", ClassificationType.NEWQUESTION)
