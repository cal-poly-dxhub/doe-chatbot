...
from common.utils import download_image_from_s3, format_template_variables, parse_qa_response, _IMG_RE, _VID_RE
    ...
    def call_text_llms(
        ...
        **kwargs: dict,
        ...
        if os.getenv("GUARDRAIL_ARN"):
            print(f'Guardrail ARN: {os.getenv("GUARDRAIL_ARN")}')
            guardrail_config = {
        ...
        try:
            ...
            if streaming_context is not None:
                try:
                    ...
                    pattern_buffer = ""
                    in_potential_pattern = False
                    
                    for event in response["stream"]:
                        if "contentBlockDelta" in event:
                            content_delta_text = event["contentBlockDelta"]["delta"]["text"]
                            
                            # Process chunk character by character to handle complex cases
                            i = 0
                            while i < len(content_delta_text):
                                char = content_delta_text[i]
                                
                                if in_potential_pattern:
                                    pattern_buffer += char
                                    ...
                                    if char == ")":
                                        ...
                                        # Check if this is an image/video pattern
                                        print(f"Checking pattern: [{pattern_buffer}]")
                                        print(f"Image regex: {_IMG_RE.pattern}")
                                        img_match = _IMG_RE.search(pattern_buffer)
                                        vid_match = _VID_RE.search(pattern_buffer)
                                        print(f"Image match: {img_match}")
                                        print(f"Video match: {vid_match}")
                                        if img_match or vid_match:
                                            print(f"Found valid pattern: [{pattern_buffer}]")
                                            transformed = parse_qa_response(pattern_buffer, do_substitute=True)
                                            print(f"After presigned substitute: [{transformed}]")   
                                            stream_llm_response(
                                            ...
                                            inference_result += transformed
                                        else:
                                            # Not a special pattern, just send through
                                            print(f"Not a valid pattern: [{pattern_buffer}]")
                                            stream_llm_response(
                                                streaming_context.connectionId,
                                                {
                                                    "chatId": streaming_context.chatId,
                                                    "messageId": streaming_context.messageId,
                                                    "chunks": [pattern_buffer],
                                                },
                                            )
                                            inference_result += pattern_buffer
                                        
                                        pattern_buffer = ""
                                elif char == "(":
                                    # Start of potential pattern
                                    in_potential_pattern = True
                                    pattern_buffer = char
                                else:
                                    # Regular text
                                    stream_llm_response(
                    ...
                    # Handle any leftover pattern buffer
                    if pattern_buffer:
                        print(f"Handling leftover pattern: [{pattern_buffer}]")
                        stream_llm_response(
                            streaming_context.connectionId,
                            {
                                "chatId": streaming_context.chatId,
                ...
                if response["stopReason"] == "guardrail_intervened":
                    print(f"Response for when Stop Reason is guardrail intervened: {response}")
                    return (response['output']['message']['content'][0]['text'], 0, 0)
                elif response["stopReason"] not in (
                    "end_turn",
                    "max_tokens",
                    "stop_sequence",
                ):
                    ...
                    inference_result = parse_qa_response(answer, do_substitute=True)
                    input_tokens, output_tokens = response["usage"]["inputTokens"], response["usage"]["outputTokens"]

            print(f"Response received from {model_config['modelId']}: {inference_result}")
