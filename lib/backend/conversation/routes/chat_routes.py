...
def list_message_sources(chat_id: str, message_id: str) -> Dict:
    ...
    for record in records:
        source_dict = record.dict()
        # Generate presigned URL if source is an S3 URI
        if source_dict.get("metadata", {}).get("source", "").startswith("s3://"):
            try:
                bucket_name, object_key = source_dict["metadata"]["source"].replace("s3://", "").split("/", 1)
                presigned_url = s3_client.generate_presigned_url(
                    "get_object",
                    Params={"Bucket": bucket_name, "Key": object_key},
                    ExpiresIn=3600,  # URL expires in 1 hour
                )
                source_dict["metadata"]["source"] = presigned_url
            except Exception as e:
                logger.error(f"Failed to generate presigned URL for {source_dict['metadata']['source']}: {e}")
