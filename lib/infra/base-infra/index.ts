...
export class BaseInfra extends Construct {
    ...
    public readonly inputAssetsBucket?: s3.IBucket;
    public readonly guardrail?: bedrock.CfnGuardrail;
    public readonly linksTable: ddb.Table;
    ...
    public constructor(scope: Construct, id: string, props: BaseInfraProps) {
        ...
        this.inputAssetsBucket = props.inputAssetsBucket;
        this.serverAccessLogsBucket =
            props.serverAccessLogsBucket ??
            new s3.Bucket(this, 'ServerAccessLogsBucket', {
        ...
        if (props.systemConfig.llmConfig.guardrailConfig) {
            ...
            this.guardrail = new bedrock.CfnGuardrail(this, 'ContentSafetyGuardrail', {
                name: `francis-chatbot-safety-${props.systemConfig.applicationName}`,
            ...
            console.log('Guardrail created with the following details:');
            console.log('Name:', this.guardrail.name);
            console.log('Blocked Input Messaging:', this.guardrail.blockedInputMessaging);
            console.log(
                'Blocked Output Messaging:',
                this.guardrail.blockedOutputsMessaging
            );
            console.log('Content Filters:', this.guardrail.contentPolicyConfig);
            console.log('PII Filters:', this.guardrail.sensitiveInformationPolicyConfig);
        ...
        this.linksTable = new ddb.Table(this, 'LinksTable', {
            partitionKey: {
                name: 'uuid',
        ...
        NagSuppressions.addResourceSuppressionsByPath(
            cdk.Stack.of(this),
            `${cdk.Stack.of(this).stackName}/${id}/ConfigTableCustomResource/CustomResourcePolicy/Resource`,
            [
                {
                    id: 'AwsSolutions-IAM5',
                    reason: 'The Custom Resource needs to be able to create and manage the ConfigTable, which requires wildcard permissions for the table name that is generated at runtime.',
                },
            ]
        );
    }
