...
import { queryKeyGenerators } from '../../hooks/chats';
...
export const ConversationView = forwardRef((props: ConversationViewProps, ref: React.ForwardedRef<HTMLDivElement>) => {
  ...
  type ListChatsResponse = { chats: ChatSummary[] };
  ...
  useEffect(() => {
    ...
    // ‼️ 1) give getQueryData a generic so it returns ListChatsResponse | undefined
    const existingChats = queryClient.getQueryData<ListChatsResponse>(queryKeyGenerators.listChats());
    if (!existingChats) return; // 2) guard against cache-miss

    const thisChat = existingChats.chats.find((c) => c.chatId === chatId);
    const title = thisChat?.title;
    if (!title?.startsWith('New Chat')) return;

    void queryClient.invalidateQueries({
      queryKey: queryKeyGenerators.listChats(),
    });
  }, [messages.length]);
      ...
      >
        ...
        {messages.map((message) => (
          <Message
            message={{
              ...message,
              // type: messages.findIndex((m) => m.messageId === message.messageId) % 2 === 1 ? 'ai' : 'human',
            }}
